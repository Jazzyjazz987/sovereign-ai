...
-- Jalon B : périmètre de connaissance. 'atelier' = interne (tout), 'teleassistance' =
-- visible aussi au prestataire externe (liste blanche).
ALTER TABLE rag.chunks ADD COLUMN IF NOT EXISTS audience TEXT NOT NULL DEFAULT 'atelier';
CREATE INDEX IF NOT EXISTS chunks_audience_idx ON rag.chunks (audience);
"""
        ...
        psycopg2.extras.execute_values(
            ...
            """INSERT INTO rag.chunks
               ...
               ON CONFLICT (chunk_id) DO UPDATE SET
                 ...
                 corpus=EXCLUDED.corpus, audience=EXCLUDED.audience, ingested_at=now()""",
            [
                (r["chunk_id"], r["doc_id"], r["proc_code"], r["titre"], r["domaine"],
                 ...
                 r["source_file"], r["text"], r["corpus"],
                 r.get("audience", "atelier"), _vec(e))
...
def _audience_clause(audience: str | None, where: list, params: list) -> None:
    """Profil 'teleassistance' => seulement les fiches en liste blanche. 'atelier' => tout."""
    if (audience or "atelier").lower() == "teleassistance":
        where.append("audience = 'teleassistance'")
...
def search(query_vec: list[float], k: int = 5, domaine: str | None = None,
    ...
    _audience_clause(audience, where, params)
...
def chunks_for_codes(codes: list[str], audience: str | None = None,
                     corpora: tuple[str, ...] = DEFAULT_CORPORA) -> list[dict]:
    """Un chunk représentatif par code de procédure (pour l'expansion `related` — C9)."""
    if not codes:
        return []
    aud = " AND audience = 'teleassistance'" if (audience or "").lower() == "teleassistance" else ""
    with connect() as c, c.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute(f"""
            SELECT DISTINCT ON (proc_code)
                   ...
                   contrainte_rgpd, source_url, source_file, related, text
            FROM rag.chunks
            WHERE proc_code = ANY(%s) AND corpus = ANY(%s){aud}
            ORDER BY proc_code,
...
def stats() -> dict:
    with connect() as c, c.cursor() as cur:
        ...
        out = {row[0]: {"chunks": row[1], "docs": row[2]} for row in cur.fetchall()}
        cur.execute("SELECT audience, count(*) FROM rag.chunks GROUP BY audience")
        out["_par_audience"] = {row[0]: row[1] for row in cur.fetchall()}
        return out
